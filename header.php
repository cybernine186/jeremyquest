...
if(basename($_SERVER["SCRIPT_FILENAME"], '.php') == "login" && isset($_GET['a']) && $_GET['a'] == "login") {
	...
	$query = "SELECT hash, id, permission_handins, permission_trades, permission_looted, permission_dropped, permission_destroyed, permission_inventory, permission_logging, permission_users, permission_connections, permission_copychar, permission_purgechar, permission_zonemerger FROM users WHERE username = '" . $uname . "'";
	...
	else
	{
		...
		else
		{
			...
			if ($row['permission_zonemerger'])
				$permission['zonemerger'] = true;
...
<?php
			...
			if ($eqcgood && ($permission['inventory'] || $permission['copychar'] || $permission['purgechar'] || $permission['zonemerger']))
			{
?>
...
<?php
					...
					if ($permission['zonemerger'])
						print "<a class='dropdown-item' href='zonemerger.php'>Merge Zone</a>";
?>
					</div>
				</li>
<?php
